...
const pluginDir = parsePluginDirArg(process.argv, "wiring");
const fsApi = createPluginFs(pluginDir);
const { readText, exists, walkFiles, listFiles } = fsApi;
...
let jsName = "";
if (exists(jsSrcDir)) {
  const jsFiles = walkFiles(jsSrcDir, [".ts", ".js"], DEFAULT_SKIP_DIRS);
...
if (supportsAndroid) {
  ...
  const files = walkFiles(androidMain, [".java", ".kt"], DEFAULT_SKIP_DIRS);
...
if (supportsIos) {
  ...
  const swiftFiles = walkFiles(scanRoot, [".swift"], DEFAULT_SKIP_DIRS);
...
if (supportsIos) {
  ...
  const podspecs = listFiles((name) => name.endsWith(".podspec"));
